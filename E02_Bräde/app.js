...
const MARKER_SLICE={circle:'markerCircle',circleFilled:'markerCircleFilled',frame:'markerFrame',dot:'markerDot',square:'markerSquare',bevel:'markerBevel'};
const MARKER_SHAPE_CLASS={circle:'circle',circleFilled:'circle-filled',frame:'frame',dot:'dot',square:'square',bevel:'bevel'};
const MARKER_COLOR_CLASS={default:'default',blue:'primary',red:'danger',green:'success',orange:'warning',purple:'purple',yellow:'yellow',cyan:'cyan',pink:'pink',white:'white',gray:'gray'};
function markerTypeFor(shape,color){
  const suffix=MARKER_COLOR_CLASS[color]||'default';
  const shapeClass=MARKER_SHAPE_CLASS[shape]||'circle';
  return {class:`marker-${shapeClass}-${suffix}`,slice:MARKER_SLICE[shape]||'markerCircle',...(shape==='dot'?{position:'above'}:{})};
}
...
let state=readState(), board=null,active='settings',busy=false,playing=false,playTimer=null,generation=0,disposing=false,htmlLayer=null,markerDragArrow=null;
let windows;
...
function dispose(){stop();generation++;busy=false;disposing=true;htmlLayer=null;markerDragArrow=null;if(board){board.cancelMoveInput();for(const ev of Object.values(POINTER_EVENTS))board.disableSquareSelect(ev);board.destroy();board=null;}disposing=false;}
function drawTabs(){
...
case 'settings':return `<section class="card"><h3>Gemensamt utseende</h3><div class="fields">${select('theme','Brädtema',[['default','Inbyggd · Default trä'],['default-contrast','Inbyggd · Default hög kontrast'],['green','Inbyggd · Grön'],['blue','Inbyggd · Blå'],['chess-club','Inbyggd · Chess club'],['chessboard-js','Inbyggd · Chessboard.js'],['black-and-white','Inbyggd · Svart och vitt'],['red','Egen · Röd'],['orange','Egen · Orange'],['purple','Egen · Lila'],['teal','Egen · Turkos']],s.theme)}${select('pieces','Pjäsuppsättning',[['standard.svg','Inbyggd · Standard'],['staunty.svg','Inbyggd · Staunty']],s.pieces)}${select('border','Ram',[['none','Ingen'],['thin','Tunn'],['frame','Bred']],s.border)}<div style="align-self:end">${check('coordinates','Visa koordinater',s.coordinates)}</div><div style="align-self:end">${check('responsive','responsive · följ containerns storlek',s.responsive)}</div><label class="wide">Brädets testbredd <output id="widthValue">${s.width} px</output><input id="width" type="range" min="120" max="1400" step="20" value="${s.width}"></label></div><p class="hint">cm-chessboard har ingen egen min/max-bredd; storleken styrs av containern. V07 låter oss därför testa 120–1400 px. Komponenten innehåller exakt sju brädteman och två pjässet; de fyra temana märkta Egen är våra tillägg.</p></section><section class="card"><h3>Koordinatnotation</h3><div class="api-values"><code>showCoordinates: ${s.coordinates}</code><code>borderType=frame → koordinater i ramen</code><code>borderType=none/thin → koordinater inne på rutorna</code><code>Vänd brädet → a–h och 1–8 vänds automatiskt</code></div><p class="hint">Det finns inga fler separata koordinatparametrar i cm-chessboard 8.14.0 än showCoordinates. Placering och färg följer ramtyp och tema.</p></section><section class="card"><h3>extensions []</h3><p class="hint"><code>extensions</code> är listan över extension-klasser som ska skapas tillsammans med brädet. I experimentet laddas Markers och Arrows som bas; andra flikar lägger till exempelvis Accessibility, PromotionDialog, PieceRotation, HtmlLayer, AutoBorderNone eller Persistence.</p><div class="api-values"><code>extensions: [{class: Markers}, {class: Arrows}, …]</code></div></section><section class="card"><h3>Förhandsvisning</h3>${commonControls()}<div class="buttons"><button id="sample">Exempelställning</button><button id="empty">Tomt bräde</button></div><p class="hint">Ställningen bevaras när du byter utseende.</p></section>`;
...
case 'markers':return `<section class="card"><h3>Rutor och symboler</h3><div class="access-grid">${check('markersEnabled','Visa markeringar',t.markersEnabled)}${check('dragArrowEnabled','Visa pil under pågående pjäsflytt',t.dragArrowEnabled)}</div><div class="fields" style="margin-top:14px">${select('markerShape','Markeringstyp',[['circle','Cirkel'],['circleFilled','Fylld cirkel'],['frame','Ram'],['dot','Prick'],['square','Fylld ruta'],['bevel','Hörn']],t.markerShape)}${select('markerColor','Markeringsfärg',[['default','Svart / standard'],['blue','Blå'],['red','Röd'],['green','Grön'],['orange','Orange'],['purple','Lila'],['yellow','Gul'],['cyan','Cyan'],['pink','Rosa'],['white','Vit'],['gray','Grå']],t.markerColor)}</div><p class="hint">Formerna täcker komponentens färdiga marker-slices, inklusive den fyllda cirkeln. V07 kompletterar dessutom med extra färger så markeringarna kan synas på fler brädteman.</p></section><section class="card"><h3>Pilar</h3><div class="fields"><label>Från ruta<input id="arrowFrom" type="text" maxlength="2" value="e2"></label><label>Till ruta<input id="arrowTo" type="text" maxlength="2" value="e4"></label></div><div style="margin-top:12px">${select('arrowType','Piltyp',[['default','Default · grön'],['success','Success · grön'],['secondary','Secondary · grå'],['warning','Warning · orange'],['info','Info · blå'],['danger','Danger · röd']],t.arrowType)}</div><div class="buttons"><button class="primary" id="addArrow">Lägg till pil utan pjäsflytt</button><button id="clearMarks">Rensa alla</button><button id="demoMarks">Visa exempel</button></div><p class="hint">Arrows-extensionen har en pilform men sex färdiga typer/stilar. När “Visa pil under pågående pjäsflytt” är aktiv visas den valda piltypen från startrutan till aktuell målruta medan du flyttar pjäsen.</p>${commonControls()}</section>`;
...
function createBoard(){
  ...
  board=new Chessboard($('board'),{position:initialPosition,orientation:t.orientation,responsive:s.responsive,assetsUrl:'./vendor/cm-chessboard/assets/',assetsCache:active==='parameters'?t.assetsCache:false,style:{cssClass:s.theme,showCoordinates:s.coordinates,borderType:s.border,aspectRatio:active==='parameters'?Number(t.aspectRatio):1,pieces:{file:`pieces/${s.pieces}`,tileSize:40},animationDuration:s.duration},extensions});
  ...
  if(active==='markers'&&t.dragArrowEnabled)board.enableMoveInput(markerMoveInputHandler);
  ...
  if(active==='fen'||active==='markers')board.enableSquareSelect(POINTER_EVENTS.pointerdown,e=>{
    ...
    if(active==='fen'){const piece=tabState().palette;board.setPiece(e.square,piece==='erase'?null:piece,false).then(()=>{update(s=>{s.tabs.fen.fen=board.getPosition();});updateInfo();$('fenInput').value=board.getPosition();message(`${piece==='erase'?'Raderade på':'Placerade pjäs på'} ${e.square}.`);}).catch(reportError);}
      ...
      if(tabState().dragArrowEnabled&&board.getPiece(e.square))return;
      if(!tabState().markersEnabled){message('Markeringar är avstängda.');return;}
      update(s=>{const tab=s.tabs.markers,a=tab.markers,i=a.findIndex(m=>m.square===e.square&&m.shape===tab.markerShape&&m.color===tab.markerColor);if(i>=0)a.splice(i,1);else a.push({square:e.square,shape:tab.markerShape,color:tab.markerColor});});
      drawMarks();message(`Markering på ${e.square} uppdaterad.`);
    }
  });
...
function drawMarks(){
  board.removeMarkers();board.removeArrows();
  if(tabState().markersEnabled){
    for(const m of tabState().markers){const type=storedMarkerType(m);if(type)board.addMarker(type,m.square);}
  }
  for(const a of tabState().arrows)board.addArrow(ARROW_TYPE[a.type]||ARROW_TYPE.default,a.from,a.to);
  if(markerDragArrow&&markerDragArrow.to)board.addArrow(ARROW_TYPE[tabState().arrowType]||ARROW_TYPE.default,markerDragArrow.from,markerDragArrow.to);
}
function markerMoveInputHandler(e){
  if(e.type===INPUT_EVENT_TYPE.moveInputStarted){markerDragArrow={from:e.squareFrom,to:null};return true;}
  if(e.type===INPUT_EVENT_TYPE.movingOverSquare){markerDragArrow=e.squareTo?{from:e.squareFrom,to:e.squareTo}:{from:e.squareFrom,to:null};drawMarks();return true;}
  if(e.type===INPUT_EVENT_TYPE.validateMoveInput)return true;
  if(e.type===INPUT_EVENT_TYPE.moveInputCanceled){markerDragArrow=null;drawMarks();return true;}
  if(e.type===INPUT_EVENT_TYPE.moveInputFinished){
    markerDragArrow=null;
    update(s=>s.tabs.markers.fen=board.getPosition());
    drawMarks();updateInfo();return true;
  }
  return true;
}
...
function bindControls(){
  ...
  bind('responsive','change',()=>{update(s=>s.settings.responsive=$('responsive').checked);render();message(`responsive är nu ${state.settings.responsive?'på':'av'}.`);});
  ...
  bind('markersEnabled','change',()=>{update(s=>s.tabs.markers.markersEnabled=$('markersEnabled').checked);drawMarks();message(`Markeringar ${tabState().markersEnabled?'visas':'är dolda'}.`);});
  bind('dragArrowEnabled','change',()=>{update(s=>s.tabs.markers.dragArrowEnabled=$('dragArrowEnabled').checked);render();});
  bind('markerShape','change',()=>update(s=>s.tabs.markers.markerShape=$('markerShape').value));bind('markerColor','change',()=>update(s=>s.tabs.markers.markerColor=$('markerColor').value));bind('arrowType','change',()=>{update(s=>s.tabs.markers.arrowType=$('arrowType').value);drawMarks();});
  ...
  bind('demoMarks','click',()=>{update(s=>{s.tabs.markers.markers=[{square:'e4',shape:'circle',color:'blue'},{square:'d4',shape:'frame',color:'red'},{square:'f3',shape:'dot',color:'green'},{square:'c6',shape:'square',color:'orange'},{square:'g5',shape:'bevel',color:'purple'},{square:'b6',shape:'circleFilled',color:'cyan'}];s.tabs.markers.arrows=[{from:'e2',to:'e4',type:'success'},{from:'g1',to:'f3',type:'info'}];});drawMarks();message('Exempelmarkeringar visas.');});
