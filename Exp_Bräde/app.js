...
function dispose(){stop();generation++;busy=false;disposing=true;if(board){board.cancelMoveInput();board.disableSquareSelect(POINTER_EVENTS.pointerdown);board.destroy();board=null;}disposing=false;}
function drawTabs(){
  ...
  for(const el of document.querySelectorAll('[data-tab]')){
    ...
    el.onkeydown=e=>{const i=TABS.findIndex(t=>t.id===active);let n;if(e.key==='ArrowRight')n=(i+1)%5;if(e.key==='ArrowLeft')n=(i+4)%5;if(e.key==='Home')n=0;if(e.key==='End')n=4;if(n!==undefined){e.preventDefault();active=TABS[n].id;render();$('tab-'+active).focus();}};
  }
...
function createBoard(){
  ...
  board=new Chessboard($('board'),{position:active==='animation'?positions[t.step]:t.fen,orientation:t.orientation,assetsUrl:'./vendor/cm-chessboard/assets/',assetsCache:false,style:{cssClass:s.theme,showCoordinates:s.coordinates,borderType:s.border,pieces:{file:`pieces/${s.pieces}`},animationDuration:s.duration},extensions:[{class:Markers,props:{autoMarkers:null}},{class:Arrows}]});
...
function updateInfo(){if(!board)return;$('fenOutput').value=board.getPosition();$('orientation').textContent=board.getOrientation()==='w'?'Vit sida nedåt':'Svart sida nedåt';if(active==='input'){drawEvents();$('undo').disabled=!tabState().history.length;}if(active==='animation')drawSteps();}
...
function drawMarks(){board.removeMarkers();board.removeArrows();for(const m of tabState().markers)board.addMarker(MARKER_TYPE[m.type],m.square);for(const a of tabState().arrows)board.addArrow(ARROW_TYPE[a.type],a.from,a.to);}
